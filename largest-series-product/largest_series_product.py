...
def largest_product(series, size):
    ...
    scan the series by sliding a window ofszie length

    - if any 0 in that window keep moving to exlude the zero...
    ...
    if not re.match(r'\A[0-9]+\Z', series) or \
       size < 0 or \
       len(series) < size:
        raise ValueError("Expect a numerical string as input")

    max_s, max_seq = 0, []
    for ix in range(len(series) - size + 1):
        seq = series[ix:ix + size]
        if '0' in seq: continue
        s = math.prod(map(lambda s: int(s),
